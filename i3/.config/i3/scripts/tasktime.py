...
class Calculator:
    ...
    def handle_tasks(self, tasks):
        ...
        for t in tasks:
            ...
            entry_date = self.internal_to_datetime(t["entry"])

            if self.print_null or tmp_seconds != 0:
                self.printer.print_task(t["description"], tmp_seconds, entry_date, status)
...
class Printer:
    ...
    def print_task(self, description, seconds, entry_date):
        raise NotImplementedError()
...
# Custom for rofi
class RofiPrinter(Printer):
    def print_header(self, project):
        print("<span color='#fad390'>" + project + "</span>")
        #separator = ['-' for c in project]
        #print(''.join(separator))
        print()

    def print_task(self, description, seconds, entry_date, status):
        time_utc = datetime.timedelta(hours=5) # hacky way to align UTC Peru time
        entry_date = entry_date - time_utc
        if status == "completed":
            status_icon = ""
        elif status == "pending":
            status_icon = ""
        ...
        print("<span color='#ff7f50'>" + status_icon + "\t" + description + "</span>")
        if seconds != 0:
            print("\t<span color='#82ccdd'>" + self.seconds_to_readable(seconds) + "</span> <span color='#999'>[" + str(entry_date) + "]</span>")

    def print_result(self, seconds):
        print()
        print("\t<span color='#fff'>" + self.seconds_to_readable(seconds) + "</span>")
        print()
...
def print_help():
    ...
    print("\t-r, --rofi\t\tPrint output in Pango format for rofi message")
...
if __name__ == "__main__":
    ...
    for i,param in enumerate(params):
        ...
        elif param == '--rofi' or param == "-r":
            c.setPrinter(RofiPrinter())
